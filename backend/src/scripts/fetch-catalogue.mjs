...
const USER_AGENT =
  "E commerce-catalogue/1.0 (educational demo project; contact: dev@nova.local)";
...
async function downloadImage(url, baseName) {
  ...
  const contentType = (response.headers.get("content-type") ?? "")
    ...
    .trim();
  const extension =
    EXTENSIONS[contentType] ?? path.extname(new URL(url).pathname) ?? ".jpg";
...
async function resolveWikiImage(page) {
  ...
  const thumbResponse = await fetch(thumbEndpoint, {
    headers: { "User-Agent": USER_AGENT },
  });
  if (thumbResponse.ok) {
...
const SHOPIFY_SOURCES = [
  {
    ...
    picks: [
      {
        match: /airdopes\s*(plus\s*)?(141|161|91|311)/i,
        category: "audio",
        tags: ["bestseller"],
      },
      { match: /rockerz\s*(450|550|558)/i, category: "audio", tags: [] },
      {
        match: /stone\s*(350|650|1200|rush)/i,
        category: "audio",
        tags: ["deal"],
      },
      {
        match: /wave\s*(call|flex|sigma|pro)/i,
        category: "wearables",
        tags: [],
      },
  ...
  },
  {
    ...
    picks: [
      {
        match: /colorfit\s*(pro|ultra|icon)/i,
        category: "wearables",
        tags: ["bestseller"],
      },
      {
        match: /buds\s*(vs|combat|xero|connect)/i,
        category: "audio",
        tags: [],
      },
      { match: /air\s*(clips|buds)/i, category: "audio", tags: ["new"] },
      {
        match: /luna\s*ring|noise\s*ring/i,
        category: "wearables",
        tags: ["featured"],
      },
      {
        match: /power\s*\d*\s*in\s*1|usb\s*hub/i,
...
function cleanTitle(rawTitle, brand) {
  ...
  name = name
    .replace(
      /\s*[-–]\s*(abhi\s*marketplace|limited\s*edition|super\s*saver|deal|combo).*$/i,
      "",
    )
    .replace(
      /\s*[-–]\s*(jet\s*black|black|white|grey|gray|blue|green|beige|silver)\s*$/i,
      "",
    )
...
async function fetchShopify(source) {
  ...
  if (!response.ok)
    throw new Error(`${source.brand} feed HTTP ${response.status}`);
...
const CATEGORY_META = [
  {
    id: "smartphones",
    name: "Smartphones",
    blurb: "Flagships and foldables",
    icon: "smartphone",
  },
  {
    id: "laptops",
    name: "Laptops",
    blurb: "Ultrabooks and workstations",
    icon: "laptop",
  },
  {
    id: "tablets",
    name: "Tablets",
    blurb: "Slates and 2-in-1s",
    icon: "tablet",
  },
  {
    id: "audio",
    name: "Audio",
    blurb: "Earbuds, headphones, speakers",
    icon: "headphones",
  },
  {
    id: "wearables",
    name: "Wearables",
    blurb: "Smartwatches and bands",
    icon: "watch",
  },
  {
    id: "smart-home",
    name: "Smart Home",
    blurb: "Speakers and hubs",
    icon: "lamp",
  },
  {
    id: "accessories",
    name: "Accessories",
    blurb: "Hubs, cables, chargers",
    icon: "router",
  },
];
...
const BRAND_META = {
  Apple: {
    ...
    blurb: "Premium ecosystem hardware.",
  },
  Samsung: {
    name: "Samsung",
    origin: "Suwon, South Korea",
    blurb: "Displays, silicon and flagships.",
  },
  boAt: {
    name: "boAt",
    origin: "Delhi, India",
    blurb: "Affordable audio and wearables.",
  },
  Noise: {
    name: "Noise",
    origin: "Gurugram, India",
    blurb: "Smartwatches and everyday audio.",
  },
};
...
async function main() {
  ...
  for (const entry of CURATED) {
    ...
    try {
      ...
      products.push(
        buildProduct(entry, [{ ...image, sourceUrl }], "wikimedia"),
      );
      console.log(
  ...
  for (const source of SHOPIFY_SOURCES) {
    ...
    for (const { raw, pick } of selected) {
      ...
      const compareAt = variant.compare_at_price
        ? toUsd(variant.compare_at_price)
        : 0;
        ...
        buildProduct(
          {
            ...
            stock: raw.variants.some((entry) => entry.available)
              ? 20 + (raw.id % 120)
              : 0,
            tags: pick.tags,
            colors: colorsFromShopify(raw),
            shortDescription:
              body.slice(0, 165) || `${name} from ${source.brand}.`,
            description:
              body.slice(0, 900) ||
              `${name}, sold and supported by ${source.brand}.`,
            highlights: (raw.tags ?? [])
      ...
      console.log(
        `   ✓ ${name.padEnd(38)} $${price} · ${images.length} image(s)`,
      );
    }
  }
  ...
  const catalogue = {
    ...
    sources: {
      wikimedia:
        "https://commons.wikimedia.org (CC-licensed product photography)",
      boAt: "https://www.boat-lifestyle.com/products.json",
      Noise: "https://www.gonoise.com/products.json",
    },
    categories: CATEGORY_META,
    brands: [...new Set(products.map((product) => product.brand))].map(
      (brand) => ({
        id: slugify(brand),
        ...BRAND_META[brand],
      }),
