  ...
  return (
    <div className="flex items-start justify-between gap-3 text-sm">
      <span
        className={cn(
          "text-ink-600",
          tone === "total" && "font-bold text-ink-900",
        )}
      >
        {label}
        {hint ? (
...
export function FreeShippingMeter({ className }) {
  ...
  const progress = Math.min(
    ((FREE_SHIPPING_THRESHOLD - totals.amountToFreeShipping) /
      FREE_SHIPPING_THRESHOLD) *
      100,
    100,
  );
...
export function PromoInput({ className }) {
  ...
  const submit = async (event) => {
    ...
    toast(
      result.message ??
        (result.ok ? "Promo applied." : "Could not apply promo"),
      {
  ...
  return (
    <div className={cn("surface-card overflow-hidden", className)}>
      ...
      <div className="space-y-3 px-5 py-4">
        {showPromo && variant === "cart" ? (
          <PromoInput className="mb-4" />
        ) : null}
